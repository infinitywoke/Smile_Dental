-- Phase 7: Add UNIQUE constraint and RPC for atomicity

-- Ensure one clinical record per appointment
ALTER TABLE clinical_records ADD CONSTRAINT clinical_records_appointment_id_key UNIQUE (appointment_id);
...
DECLARE
    ...
    v_tooth JSONB;
BEGIN
    -- 1. Get tenant_id and patient_id from the appointment
    -- This implicitly respects RLS because SECURITY INVOKER is used
    ...
    -- 3. Update appointment status to IN_PROGRESS if CHECKED_IN
    UPDATE appointments
    SET status = 'IN_PROGRESS', updated_at = NOW()
    WHERE id = p_appointment_id AND status IN ('CHECKED_IN', 'SCHEDULED', 'CONFIRMED');

    -- 4. Upsert clinical_record
    ...
    -- 5. Delete existing teeth to replace them
